...
appId = b"ai.polyguard.secureline"
teamId = b"HY6Z2F5595"
...
class noncePayload(univ.Sequence):
    componentType = namedtype.NamedTypes(
        namedtype.NamedType('nonce',
                            univ.OctetString().subtype(
                                implicitTag=tag.Tag(tag.tagClassContext,
                                                    tag.tagFormatConstructed, nonceTag))),
    )
...
class AuthenticatorData:
    ...
    def __init__(self, data: bytes):
        ...
        self._rpid, self._flags, self._counter = struct.unpack(
            AuthenticatorData._authData_format, data[_offset: _offset + _size])
        _offset += _size

        if self._flags & AuthenticatorData.FLAG_AT:
            _size = struct.calcsize(AuthenticatorData._authData_format1)
            self._aaguid, _credentialIdLength = struct.unpack(
                AuthenticatorData._authData_format1, data[_offset: _offset + _size])
            _offset += _size

            __authData_format = AuthenticatorData._authData_format2 % _credentialIdLength
            _size = struct.calcsize(__authData_format)
            self._credentialId = struct.unpack(
                __authData_format, data[_offset: _offset + _size])[0]
            _offset += _size
    ...
    def __repr__(self) -> str:
        ...
        if self._credentialId is not None:
            _s += ', %r' % self._credentialId
        if self._credentialPublicKey is not None:
...
def verifyAppAttestAttestation(attestation: bytes,
                               clientDataHash: bytes,
                               keyId: bytes) -> Tuple[ec.EllipticCurvePublicKey, bytes]:
